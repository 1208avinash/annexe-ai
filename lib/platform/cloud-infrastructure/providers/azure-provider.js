...
export default class AzureProvider {
  constructor({
    ...
    this.container = new ContainerAdapter({ runtime: "azure-functions", image: "annexe-ai/azure-cloud" });
    this.docker = new DockerManager({ image: "annexe-ai/azure-cloud" });
    this.kubernetes = new KubernetesAdapter({ namespace: "annexe-ai-azure", cluster: "aks-ready" });
    this.environment = new EnvironmentManager({ region, availabilityZone, dataResidency, complianceLocation });
    this.storageAdapter = new S3StorageAdapter({ workspaceRoot, bucket: "annexe-ai-azure" });
