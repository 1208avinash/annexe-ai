...
import CustomerPortalOrchestrator from "./customer-portal/customer-portal-orchestrator.js";
...
function buildPortalData({ company, benchmarks, customerPortal, adminDashboard }) {
    ...
    const portalJourney = customerPortal?.intake?.journey ?? [
        "Visitor",
        "Create Account",
    ...
    const portalOverview = customerPortal?.tracking?.projectOverview ?? {
        status: "Completed",
        progress: 100,
        estimatedCompletion: "Completed",
        currentDepartment: "Delivery"
    };
    const portalPaymentCenter = customerPortal?.paymentCenter ?? {
        invoice: { total: company.estimation?.estimation?.estimatedCost ?? 0, advance: 0, remaining: 0 },
        advancePaymentStatus: "READY",
        remainingBalance: 0,
        upgradePayments: "50% advance / 50% completion"
    };
    const portalUpgradeCenter = customerPortal?.upgradeCenter ?? {
        improvementRecommendations: company.evolutionDepartment?.recommendation?.prioritizedImprovements ?? [],
        upgradeRequests: company.upgradeDepartment?.plan?.implementationPhases ?? [],
        evolutionSuggestions: company.evolutionDepartment?.roadmap?.immediateImprovements ?? []
    };
    const portalBillingModel = customerPortal?.paymentCenter?.billingModel ?? {
        initialProject: { advance: 50, completion: 50 },
        upgrade: { advance: 50, completion: 50 },
        maintenance: { type: "Monthly subscription" }
    };
    const customerSaasCapabilities = customerPortal?.report?.saasCapabilities ?? [
        "Customer onboarding",
        "Proposal review",
        "Project approval",
        "Project tracking",
    ...
    const adminViews = adminDashboard?.views ?? {
        ceo: { revenue: "$0", projects: 1, customers: 1 },
        sales: { leads: 1, proposals: 1, conversions: 1 },
        engineering: { activeProjects: 1, workload: "Balanced" },
        qa: { certifications: 1 },
        security: { vulnerabilities: 0 },
        devops: { deployments: 1 },
        upgrade: { recurringRevenue: "Maintenance and upgrade subscriptions" }
    };
    ...
    return {
        ...
        summary: {
            ...
            stage: portalOverview.status,
            progress: portalOverview.progress,
            currentDepartment: portalOverview.currentDepartment,
            estimatedCompletion: portalOverview.estimatedCompletion,
        ...
        navigation: [
            ...
            { path: "/billing", label: "Billing Center" },
            { path: "/upgrade-center", label: "Upgrade Center" },
        ...
        customerPortal: {
            ...
            projectOverview: portalOverview,
            companyActivity: customerPortal?.tracking?.companyActivity ?? [
                "CEO analysis",
                "Product planning",
                "Architecture",
                "Engineering",
            ...
            deliverables: customerPortal?.proposalReview?.deliverables ?? [
                "proposal",
                "architecture document",
                "reports",
                "certificates"
            ],
            paymentCenter: portalPaymentCenter,
            upgradeCenter: portalUpgradeCenter,
            billingModel: portalBillingModel
        },
        customerJourney: portalJourney,
        billingModel: portalBillingModel,
        saasCapabilities: customerSaasCapabilities,
        architecture: {
        ...
        reports: [
            { name: "Commercial Operating System", path: "/reports/platform/commercial-operating-system-report.json" },
            { name: "Billing Model", path: "/reports/platform/billing-model.json" },
...
function buildPortalFiles({ projectName, data }) {
    ...
    const pageConfigs = {
        ...
        dashboard: {
            ...
            description: "Project overview, company activity, deliverables, payments, and upgrades in one view.",
            sections: [
                { kicker: "Project Overview", title: data.customerPortal.projectOverview.status, description: `Progress ${data.customerPortal.projectOverview.progress}% Ã‚Â· Current department ${data.customerPortal.projectOverview.currentDepartment} Ã‚Â· Estimated completion ${data.customerPortal.projectOverview.estimatedCompletion}.` },
                { kicker: "AI Company Activity", title: "Autonomous company layers", description: data.customerPortal.companyActivity.join(" Ã¢â‚¬Â¢ ") },
                { kicker: "Deliverables", title: "Proposal and certificates", description: data.customerPortal.deliverables.join(" Ã¢â‚¬Â¢ ") },
                { kicker: "Payment Center", title: data.customerPortal.paymentCenter.advancePaymentStatus, description: `Invoice total ${data.customerPortal.paymentCenter.invoice.total}, advance ${data.customerPortal.paymentCenter.invoice.advance}, remaining ${data.customerPortal.paymentCenter.remainingBalance}.` },
                { kicker: "Upgrade Center", title: "Lifecycle improvements", description: data.customerPortal.upgradeCenter.improvementRecommendations.concat(data.customerPortal.upgradeCenter.evolutionSuggestions).join(" Ã¢â‚¬Â¢ ") || "Upgrade recommendations ready." }
            ]
        },
        projects: {
            title: "Projects",
        ...
        generation: {
            ...
            sections: [
                ...
                { kicker: "Logs", title: "Execution logs", description: data.generation.logs.join(" Ã¢â‚¬Â¢ ") }
            ]
        },
        deployment: {
            title: "Deployment",
        ...
        admin: {
            ...
            description: "CEO, Sales, Engineering, QA, Security, DevOps, and Upgrade views.",
            sections: [
                { kicker: "CEO View", title: data.adminDashboard.ceo.revenue, description: `${data.adminDashboard.ceo.projects} projects and ${data.adminDashboard.ceo.customers} customers.` },
                { kicker: "Sales View", title: `${data.adminDashboard.sales.leads} leads`, description: `${data.adminDashboard.sales.proposals} proposals and ${data.adminDashboard.sales.conversions} conversions.` },
                { kicker: "Engineering View", title: `${data.adminDashboard.engineering.activeProjects} active projects`, description: `Workload ${data.adminDashboard.engineering.workload}.` },
                { kicker: "QA View", title: `${data.adminDashboard.qa.certifications} certification(s)`, description: "QA certificates and quality gate status." },
                { kicker: "Security View", title: `${data.adminDashboard.security.vulnerabilities} vulnerabilities`, description: "Security posture and risk exposure." },
                { kicker: "DevOps View", title: `${data.adminDashboard.devops.deployments} deployments`, description: "Deployment operations and observability." },
                { kicker: "Upgrade View", title: data.adminDashboard.upgrade.recurringRevenue, description: "Recurring revenue from upgrades and maintenance." }
            ]
        },
        billing: {
            title: "Payment Center",
            description: "Advance payments, completion payments, and monthly subscriptions.",
            sections: [
                { kicker: "Initial Project", title: "50% advance / 50% completion", description: `Advance ${data.customerPortal.billingModel.initialProject.advance}% and completion ${data.customerPortal.billingModel.initialProject.completion}%.` },
                { kicker: "Upgrade", title: "50% advance / 50% completion", description: `Upgrade billing follows the same policy.` },
                { kicker: "Maintenance", title: data.customerPortal.billingModel.maintenance.type, description: "Monthly subscription billing for support and lifecycle operations." }
            ]
        },
        "upgrade-center": {
            title: "Upgrade Center",
            description: "Improvement recommendations, upgrade requests, and evolution suggestions.",
            sections: [
                { kicker: "Recommendations", title: `${data.customerPortal.upgradeCenter.improvementRecommendations.length} suggestions`, description: data.customerPortal.upgradeCenter.improvementRecommendations.join(" Ã¢â‚¬Â¢ ") || "Ready for recommendations." },
                { kicker: "Upgrade Requests", title: `${data.customerPortal.upgradeCenter.upgradeRequests.length} requests`, description: data.customerPortal.upgradeCenter.upgradeRequests.join(" Ã¢â‚¬Â¢ ") || "No upgrade requests yet." },
                { kicker: "Evolution", title: `${data.customerPortal.upgradeCenter.evolutionSuggestions.length} suggestions`, description: data.customerPortal.upgradeCenter.evolutionSuggestions.join(" Ã¢â‚¬Â¢ ") || "Evolution suggestions available." }
            ]
        },
        benchmark: {
...
function buildCommercialReports({ company, benchmarks, platformRoot, customerPortal, adminDashboard }) {
    const readinessScore = Math.min(
        100,
        Math.round(
            0.4 * (customerPortal?.report?.platformReadiness ?? 100) +
    ...
    return {
        "reports/platform/commercial-operating-system-report.json": {
            customerJourney: customerPortal?.report?.customerJourney ?? [],
            revenueModel: customerPortal?.report?.revenueModel ?? {},
            platformReadiness: customerPortal?.report?.platformReadiness ?? readinessScore,
            saasCapabilities: customerPortal?.report?.saasCapabilities ?? [],
            adminDashboard: adminDashboard?.views ?? null,
            generatedAt: new Date().toISOString()
        },
        "reports/platform/billing-model.json": customerPortal?.paymentCenter?.billingModel ?? {
            initialProject: { advance: 50, completion: 50 },
            upgrade: { advance: 50, completion: 50 },
            maintenance: { type: "Monthly subscription" }
        },
        ...
        "reports/platform/commercial-operating-system-summary.json": {
            platformReadinessPercent: readinessScore,
            customerJourneyLength: customerPortal?.report?.customerJourney?.length ?? 0,
            saasCapabilities: customerPortal?.report?.saasCapabilities ?? [],
            generatedAt: new Date().toISOString()
        }
    };
...
export async function runCommercialSaaSPlatform({
    ...
    const customerPortalOrchestrator = new CustomerPortalOrchestrator();
    const adminDashboardOrchestrator = new AdminDashboardOrchestrator();
    const customerPortal = customerPortalOrchestrator.processRequest({
        company,
        benchmarks,
        platformRoot,
        platformReadiness: company.validation?.frontend?.build && company.validation?.backend?.pytest ? 100 : 90,
        saasCapabilities: company.composition?.capabilities ?? []
    });
    const adminDashboard = adminDashboardOrchestrator.processRequest({
        company,
        benchmarks
    });
    const data = buildPortalData({ company, benchmarks, customerPortal, adminDashboard });
    const portalFiles = buildPortalFiles({ projectName: "ANNEXE AI Commercial SaaS Platform", data });
    const reportFiles = buildCommercialReports({ company, benchmarks, platformRoot, customerPortal, adminDashboard });
    ...
    const publicReportCopies = [
        [path.join(platformRoot, "reports", "platform", "commercial-operating-system-report.json"), "frontend/public/reports/commercial-operating-system-report.json"],
        [path.join(platformRoot, "reports", "platform", "billing-model.json"), "frontend/public/reports/billing-model.json"],
    ...
    writeJson(path.join(platformRoot, "reports", "commercial-platform-report.json"), {
        ...
        benchmarks,
        customerPortal: customerPortal.reportPaths,
        data: {
